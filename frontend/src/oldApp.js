...
const initialFormState = {
    ...
    doctorInfo: { 
        name: 'Dr. Doctor Name', 
        qualification: 'Education Qualification., Speciality',
        regdNo: 'Tamil Nadu Medical Council REGD. No. 00000',
        clinicAddress: 'Clinic Address and Pincode with clinic Time Slots'
    },
    patientInfo: { 
        name: '', 
        age: '', 
...
function App() {
    ...
    const isEditable = (report) => {
        ...
        console.log('Checking edit permissions:', {
            ...
            currentHospitalId: currentAdmin.hospital.id,
            reportDate: new Date(report.createdAt).toISOString().slice(0, 10),
            today: new Date().toISOString().slice(0, 10)
        });
        
        // Check if same hospital - compare string values
        const isSameHospital = report.hospitalInfo?.hospitalId?.toString() === currentAdmin.hospital.id.toString();
        ...
        console.log('Edit permission result:', { isSameHospital, isSameDay, canEdit });
                ...
                {activeTab === 'form' && (
                    <div className="form-section">
                        <div className="section-header">
                            <h2>{formData._id ? 'Edit Prescription' : 'Create New Prescription'}</h2>
                            {formData._id && (
                ...
                {activeTab === 'reports' && (
                    <div className="reports-section">
                        ...
                        <div className="reports-stats">
                            ...
                            <div className="stat-card">
                                <div className="stat-number">
                                    {new Set(reports.map(r => r.hospitalInfo?.hospitalName).filter(Boolean)).size}
                                </div>
                        ...
                        <div className="reports-list">
                            {filteredReports.length === 0 ? (
                                ...
                                filteredReports.map(report => {
                                    ...
                                    const today = new Date().toISOString().slice(0, 10);
                                    const reportDate = new Date(report.createdAt).toISOString().slice(0, 10);
                                    const isOldPrescription = isSameHospital && today !== reportDate;
                                            ...
                                            <div className="report-header">
                                                <div>
                                                    ...
                                                    <div className="hospital-badge">
                                                        ...
                                                        {isSameHospital && ' (Your Hospital)'}
                                                    </div>
                                                </div>
                                                <span className={`status-badge ${isEditable(report) ? 'status-editable' : 'status-locked'}`}>
                                                    {isEditable(report) ? '✏️ Editable' : 
                                                     isOldPrescription ? '🔒 Locked' : '👁️ View Only'}
                                                </span>
                                            </div>
                                    ...
                                    <div className="details-header">
                                        <div>
                                            ...
                                            <div className="hospital-badge large">
                                                ...
                                                {selectedReport.hospitalInfo?.hospitalId?.toString() === currentAdmin.hospital.id.toString() && ' (Your Hospital)'}
                                            </div>
                                        </div>
                                        <button 
